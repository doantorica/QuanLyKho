...
class InventoryManager:
    def __init__(self, connection):
        ...
        self.cache_changed = {'items': False, 'machines': False, 'sales': False, 'imports': False,
                             'photocopy_sales': False, 'rentals': False, 'maintenance': False}
        ...
        try:
            ...
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    password VARCHAR(64),  # Lưu hash SHA-256
            ...
            """)
            # Tạo bảng photocopy_machines với trạng thái NOT NULL và mặc định 'Trong Kho'
            self.cursor.execute("""
            ...
            self.cursor.execute("SHOW COLUMNS FROM photocopy_machines LIKE 'trang_thai'")
            column_info = self.cursor.fetchone()
            if column_info and "YES" in column_info[2].upper():
                print("Cột trang_thai cho phép NULL, đang cập nhật dữ liệu trước khi áp dụng NOT NULL...")
                self.cursor.execute(
                    "UPDATE photocopy_machines SET trang_thai = 'Trong Kho' WHERE trang_thai IS NULL OR trang_thai = ''")
                self.cursor.execute(
                    "ALTER TABLE photocopy_machines MODIFY COLUMN trang_thai VARCHAR(50) NOT NULL DEFAULT 'Trong Kho'")
            # Tạo bảng photocopy_sales_history với cột gia_nhap
            self.cursor.execute("""
    ...
    def authenticate_user(self, username, password):
        """Xác thực người dùng và trả về vai trò (role)"""
        try:
            hashed_password = hashlib.sha256(password.encode()).hexdigest()
            self.cursor.execute("SELECT role FROM users WHERE username = %s AND password = %s", (username, hashed_password))
    ...
    def fetch_all_items(self, offset=0, limit=20, force_refresh=False):
        ...
        if force_refresh or self.items_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['items']:
            try:
                query = """
    ...
    def fetch_all_photocopy_machines(self, offset=0, limit=20, force_refresh=False, include_sold=False):
        current_time = time.time()
        if force_refresh or self.machines_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['machines']:
            try:
                if include_sold:
                    # Lấy cả máy trong kho và máy đã bán
                    query = """
                    ...
                    # Chỉ lấy máy trong kho (logic hiện tại)
                    query = """
                ...
                print(f"Đã làm mới danh sách máy photocopy{' (bao gồm máy đã bán)' if include_sold else ''}: {len(self.machines_cache)} mục (offset={offset}, limit={limit})")
            except mysql.connector.Error as err:
    ...
    def fetch_available_photocopy_machines(self, offset=0, limit=20, force_refresh=False):
        ...
        if force_refresh or self.machines_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['machines']:
            try:
                ...
                self.last_fetch_time = current_time
                self.cache_changed['machines'] = False
                print(f"Đã làm mới danh sách máy photocopy có sẵn: {len(self.machines_cache)} mục (offset={offset}, limit={limit})")
            except mysql.connector.Error as err:
    ...
    def fetch_rental_history(self, force_refresh=False):
        current_time = time.time()
        if force_refresh or self.rental_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['rentals']:
            try:
                query = """
    ...
    def backup_all_data(self, backup_path):
        """Backup tất cả dữ liệu vào file Excel trước khi xóa"""
        try:
            tables = {
                ...
                for table_name, data in tables.items():
                    if data:
                        ...
                        if table_name == 'vat_tu':
                            ...
                            df = pd.DataFrame(data, columns=['Loại', 'Số Lượng', 'Ngày Bán', 'Giá Bán', 'Doanh Thu', 'Tên Khách Hàng', 'SĐT'])
                            ...
                            df = pd.DataFrame(data, columns=['ID', 'Loại Máy', 'Tên Máy', 'Số Counter', 'Trạng Thái', 'Ngày Nhập', 'Giá Nhập', 'Số Serial'])
                        elif table_name == 'photocopy_sales_history':
                            df = pd.DataFrame(data, columns=['Tên Máy', 'Ngày Bán', 'Giá Bán', 'Tên Khách Hàng', 'SĐT'])
                        elif table_name == 'rental_history':
                            df = pd.DataFrame(data, columns=['Tên Máy', 'Tên Khách Hàng', 'SĐT', 'Ngày Bắt Đầu', 'Ngày Kết Thúc', 'Giá Thuê', 'Ngày Trả', 'Counter Trả', 'Khách Hàng Trả', 'SĐT Trả'])
    ...
    def clear_all_data(self):
        """Xóa toàn bộ dữ liệu (giữ nguyên bảng users)"""
        try:
            tables = [
                'counter_history', 'maintenance_history', 'rental_history',
                'photocopy_sales_history', 'photocopy_machines', 'import_history',
                'sales_history', 'vat_tu'
            ]
            for table in tables:
    ...
    def fetch_sales_history(self, force_refresh=False):
        current_time = time.time()
        if force_refresh or self.sales_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['sales']:
            try:
                query = """
    ...
    def fetch_import_history(self, force_refresh=False):
        current_time = time.time()
        if force_refresh or self.import_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['imports']:
            try:
                query = """
    ...
    def fetch_photocopy_sales_history(self, force_refresh=False):
        current_time = time.time()
        if force_refresh or self.photocopy_sales_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['photocopy_sales']:
            try:
                query = """
    ...
    def fetch_maintenance_history(self, machine_id=None, force_refresh=False):
        current_time = time.time()
        if force_refresh or self.maintenance_cache is None or (current_time - self.last_fetch_time > self.cache_timeout) or self.cache_changed['maintenance']:
            try:
                query = """
    ...
    def export_to_excel(self, file_path, is_history=False, is_photocopy=False):
        try:
            if is_photocopy:
                if is_history:
                    if 'rental' in file_path.lower() or 'rental' in file_path.lower():
                        ...
                        for row in data:
                            for i in range(len(row)):
                                if row[i] is None:
                                    row[i] = ""
                        df = pd.DataFrame(data, columns=['Tên Máy', 'Tên Khách Hàng', 'SĐT', 'Ngày Bắt Đầu', 'Ngày Kết Thúc',
                                                        'Giá Thuê', 'Ngày Trả', 'Counter Trả', 'Khách Hàng Trả', 'SĐT Trả'])
                    else:
                    ...
                    for row in data:
                        for i in range(len(row)):
                            if row[i] is None:
                                row[i] = ""
                    df = pd.DataFrame(data, columns=['ID', 'Loại Máy', 'Tên Máy', 'Số Counter', 'Trạng Thái', 'Ngày Nhập',
                                                    'Giá Nhập', 'Số Serial'])
            else:
                if is_history:
                    ...
                    for row in data:
                        for i in range(len(row)):
                            if row[i] is None:
                                row[i] = ""
                    df = pd.DataFrame(data, columns=['Loại', 'Số Lượng', 'Ngày Bán', 'Giá Bán', 'Doanh Thu', 'Tên Khách Hàng',
                                                    'SĐT'])
                else:
    ...
    def get_photocopy_rental_stats(self):
        try:
            self.cursor.execute("SELECT COUNT(id), SUM(rental_price * DATEDIFF(end_date, start_date)) FROM rental_history")
            result = self.cursor.fetchone()
